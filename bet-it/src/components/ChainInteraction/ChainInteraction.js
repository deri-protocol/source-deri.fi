...
export default function ChainInteraction({title,status = 'pending',isApproved,approved,close,direction,options={}}){
  ...
  return (<TransactionBox title={title} rgb={data['rgb']} status={status} approved={approved} options={options} direction={direction} subTitle={data['subTitle']} desc={data['desc']} icon={data['icon']} close={close}/>)
}
